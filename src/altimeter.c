...
#define MPL3115A2_SDA 0
#define MPL3115A2_SCL 1
#define MPL3115A2_INT1 _u(2)
...
void mpl3115a2_init() {
    ...
    gpio_init(MPL3115A2_INT1);
    gpio_pull_up(MPL3115A2_INT1); // pull it up even more!
    
    // set as altimeter with oversampling ratio of 128
    uint8_t buf[] = {MPL3115A2_CTRLREG1, 0xB8};
    i2c_write_blocking(i2c0, MPL3115A2_ADDR, buf, 2, false);
...
void mpl3115a2_read_raw(int32_t *altitude) {
    ...
    i2c_write_blocking(i2c0, MPL3115A2_ADDR, &reg, 1, true); // keep control of bus
    i2c_read_blocking(i2c0, MPL3115A2_ADDR, &p_msb, 1, false);
    
    reg = MPL3115A2_OUT_P_CSB;
    i2c_write_blocking(i2c0, MPL3115A2_ADDR, &reg, 1, true); // keep control of bus
    i2c_read_blocking(i2c0, MPL3115A2_ADDR, &p_csb, 1, false);

    reg = MPL3115A2_OUT_P_LSB;
    i2c_write_blocking(i2c0, MPL3115A2_ADDR, &reg, 1, true); // keep control of bus
    i2c_read_blocking(i2c0, MPL3115A2_ADDR, &p_lsb, 1, false);
